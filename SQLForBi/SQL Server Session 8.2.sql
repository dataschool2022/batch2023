...
--- Variable : A Memory Location  We can Store Any Type Of Data In Variable 
	   ...
	   SET @number2 = 4
		...
		print 'Number1 : Before Vairable Swapping: ' + cast(@number1  as varchar) -- 125  
		print 'Number2 : Before Variable Swapping: ' + cast(@number2 as varchar)  -- 4  

-- Variable Swapping 

		declare @number3 int 
		
		 SET @number3 = @number1  -- 125
		 SET @number1 = @number2  -- 4
		 SET @number2 = @number3  -- 125

		print 'Number1 : After Vairable Swapping: ' + cast(@number1  as varchar) -- 125  
		print 'Number2 : After Variable Swapping: ' + cast(@number2 as varchar)  -- 4  
...
---------------- TO CREATE A SIMPLE PROCEDURE ----------------------

CREATE PROC get_allemps
...
------------------------ TO ALTER EXISTING PROCEDURE ----------------------
-- Modifying Existing Procedure 

ALTER PROC get_allemps
...
exec emp_payment_info

SELECT @@SERVERNAME

select * from emp 

insert into emp values(9007,'Prakash','Analyst',7902,getdate(),25000,2500,10)
...
select * from salesorders where region = @region

end 



exec orders_byregion 'east'

exec orders_byregion 'west'


------------ using multipule parameters ------------------
CREATE PROC orders_byregion2 
...
select * from salesorders where region in(@region)
end 

exec orders_byregion2 'east'

------------ using multipule parameters ------------------


--- lOOPS 
--- How to Delete every 4th row/record 

---- To Generate Numbers in Sequence
...
-- how to delete every 4th record 
...
--  Step 1 Generate Row Numbers For Emp Table

With EmpInfo as
(
SELECT *,ROW_NUMBER() over(ORDER BY EMPNO) AS Row_Number FROM EMP
							
							)select * from empinfo where row_number = 4

--- to delete specific records

		delete from emp where row_number = 4
---
...
Declare @my_number int 
 ...
 while @my_number <=  (	select count(*) as Total_rows from emp)
			begin
			print @my_number 
		
			begin 
			With empSpecificDelete 
				as(
					select *,ROW_NUMBER() over(order by empno) as row_number from emp

					)delete from empSpecificDelete where row_number = @my_number
					
					--select * from empSpecificDelete where row_number = @my_number
					end
						SET @my_number = @my_number + 4
			end
